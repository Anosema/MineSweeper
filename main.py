...
colors = ["blue", "green", "yellow", "orange", "red", "darkred", "purple", "black", "white"]
class Cell(QWidget):
	def __init__(self, p, *cos):
		...
		self.near_bomb = -1
		self.setStyleSheet("border: 1px solid rgb(18, 18, 18)")
	...
	def check(self):
		if not self.checked:
			...
			self.label.setStyleSheet("font-size: " + str(min([self.width(), self.height()])) + "px;\ncolor: " + colors[self.near_bomb])

			if not self.near_bomb:
				for cos in adj_cells:
					if 0 <= cos[1] < len(self.parent().grid) and 0 <= cos[0] < len(self.parent().grid[0]) and not self.parent().grid[cos[0]][cos[1]].checked:
...
class MainWindow(QMainWindow):
	def __init__(self):
		...
		self.grid = [[Cell(self, x, y) for x in range(5)] for y in range(5)]
		b = 0
		while b != 5:
			...
			if not c.bombed:
				...
				b += 1
	def resizeEvent(self, e):
		w, h = int(self.width()/len(self.grid)), int(self.height()/len(self.grid[0]))
		for i in self.grid:
			for c in i:
				c.setGeometry(c.x*w, c.y*h, w, h)
				# c.label.setStyleSheet("font-size: "+ str(min([c.width(), c.height()])) + "px")
